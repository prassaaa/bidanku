...
function insertPengaduan($data) {
    ...
    $id = $data['id'];
    $nk = htmlspecialchars($data["nama"]);
    $jk = htmlspecialchars($data["Jenis_kelamin"]);
    $ak = htmlspecialchars($data["alamat"]);
    $uk = htmlspecialchars($data["usia"]);
    $ket = mysqli_real_escape_string($conn, $data["ket"]);
    $status = "Sedang diajukan";
    $ket_petugas = "-";
    $tgl_lapor = date("Y-m-d");

    mysqli_query($conn, "INSERT INTO pengaduan VALUES('$id', '$nk', '$jk', '$ak', '$uk', '$ket', '$status', '$ket_petugas', '$tgl_lapor')");
